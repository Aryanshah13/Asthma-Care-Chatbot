...
# Load data
df = pd.read_csv(r"C:\Users\kumaw\Downloads\Asthma-Care-Chatbot-main\Asthma-Care-Chatbot-main\processed-data.csv")
...
# Function to perform prediction with adjusted threshold for recall
def predict_asthma():
    try:
        name = name_entry.get()
        ...
        if tiredness == dry_cough == difficulty_breathing == sore_throat == nasal_congestion == runny_nose == 0:
            ...
            if check_threshold(symptoms, age, gender):
                ...
                # This example uses a 0.3 threshold, assuming index 1 is the positive class
                # You may need to adjust this threshold based on your needs and model performance
                threshold = 0.3
                severity_prediction = 1 if probas[0][1] >= threshold else 0
...
chatbot_image = Image.open(r"C:\Users\kumaw\Downloads\Asthma-Care-Chatbot-main\Asthma-Care-Chatbot-main\friendly-chatbot.jpg")
...
# Assume y_test are the true labels, and predictions are the model's predictions
predictions = model.predict(x_test)

accuracy = accuracy_score(y_test, predictions)
precision = precision_score(y_test, predictions, average='weighted')  # Use 'binary' for binary classification
recall = recall_score(y_test, predictions, average='weighted')
f1 = f1_score(y_test, predictions, average='weighted')
# For AUC-ROC, you need probability scores of the positive class, applicable for binary classification
# roc_auc = roc_auc_score(y_test, model.predict_proba(x_test)[:, 1])

print(f"Accuracy: {accuracy}\nPrecision: {precision}\nRecall: {recall}\nF1 Score: {f1}")
# Print AUC-ROC if applicable
